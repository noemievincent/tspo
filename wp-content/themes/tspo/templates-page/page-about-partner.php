...
?>
	<div class="partner grid-default mt-12 rl:mt-24">
		<div
			...
			<div
				class="w-full items-center grid grid-cols-1 md:grid-cols-2 rg:grid-cols-3 rl:grid-cols-4 lg:grid-cols-5 2xl:grid-cols-6 rl:gap-12 gap-8">
				<?php while ( $partners->have_posts() ): $partners->the_post(); ?>
					<article aria-labelledby="<?= sanitize_title( get_the_title() ) ?>"
					         class="group h-full flex items-center justify-center">
                        <div class="w-full h-full relative flex flex-col-reverse items-center gap-3">
                            <h2 id="<?= sanitize_title( get_the_title() ) ?>"
                                class="text-xl text-center <?= get_field( 'link' ) ? 'group-hover:text-orange group-focus:text-orange transition-colors' : '' ?>"><?= get_the_title() ?></h2>
                            <div class="max-w-[200px] rg:max-w-[400px] w-full h-full flex items-center justify-center">
